...
export const addTeamMemberResponse = (teamMember) => ({ type: ADD_TEAM_MEMBER_RESPONSE, teamMember });
...
export function deleteTeamMember(teamMembers) {
  return function (dispatch) {
    ...
    return new Promise((resolve) => setTimeout(resolve, 500)).then(() => {
      dispatch(deleteTeamMemberResponse(teamMembers));
      toast.info('Team Member Deleted Successfully');
    });
  };
}
...
export function addTeamMember(teamMember) {
  return function (dispatch) {
    ...
    return new Promise((resolve) => setTimeout(resolve, 500)).then(() => {
      dispatch(addTeamMemberResponse(teamMember));
